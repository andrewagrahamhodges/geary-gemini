...
const TOOLS = [
  {
    name: 'list_emails',
    description: 'List emails in the current folder. Returns an array of email objects with id, subject, from, date, and preview.',
    inputSchema: {
      type: 'object',
      properties: {
        limit: {
  ...
  },
  {
    ...
    description: 'Get the full content of the currently selected email in Geary. Returns the email with id, subject, from, to, cc, date, and body.',
    inputSchema: {
      type: 'object',
      properties: {}
    }
  ...
  {
    name: 'read_email',
    description: 'Read a specific email by its ID. Returns the full email content with id, subject, from, to, cc, date, and body.',
    inputSchema: {
      type: 'object',
      properties: {
        email_id: {
  ...
  {
    name: 'search_emails',
    description: 'Search emails in the current folder. Supports queries like "from:bob", "to:alice", "subject:invoice", or plain text search.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
          type: 'string',
          description: 'Search query (e.g., "from:bob", "subject:meeting", or "hello world")'
        }
      },
      required: ['query']
    }
  ...
  {
    name: 'select_email',
    description: 'Select an email in Geary\'s UI by its ID. This will highlight and display the email.',
    inputSchema: {
      type: 'object',
      properties: {
        email_id: {
...
async function initDBus() {
  ...
  try {
    ...
    throw new Error(`Failed to connect to Geary D-Bus service: ${error.message}. Is Geary running?`);
  }
}
...
async function handleToolCall(name, args) {
  ...
  switch (name) {
    ...
    case 'select_email': {
      ...
      return { success, message: success ? 'Email selected' : 'Email not found' };
    }
